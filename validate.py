...
if __name__ == "__main__":
    ...
    for i, ds_name in enumerate(datasets):
        ...
        plots[i][0].scatter(X[:,0],X[:,1],c= y,marker= 'o', cmap="copper")
        plots[i][0].set_title("ground truth for %s" %ds_name)

        plots[i][1].scatter(X[:,0],X[:,1],c= y_km,marker= 'o', cmap="copper")
        plots[i][1].set_title("kmeans clusteration for %s" %ds_name)

        plots[i][2].scatter(X[:,0],X[:,1],c= y_rbf,marker= 'o', cmap="copper")
        plots[i][2].set_title("rbf clusteration for %s" %ds_name)

        plots[i][3].scatter(X[:,0],X[:,1],c= y_knn,marker= 'o', cmap="copper")
